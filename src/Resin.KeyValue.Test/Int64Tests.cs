namespace Resin.KeyValue.Test
{
    ...
    public sealed class Int64Tests
    {
        ...
        public void ByteArrayWriter_CanThrowOutOfPageStorageException()
        {
            ...
            {
                var writer = new Int64Writer(keyStream, valueStream, addressStream, pageSize);
        ...
        public void ByteArrayReader_CanReadAndReturnEmptySpanIfNoMatch()
        {
            ...
            using (var addressStream = new MemoryStream())
            {
                ...
                for (int i = 0; i < testCount; i++)
                {
                    writer.TryPut(key: i, value: BitConverter.GetBytes((long)i));
                    testCases.Add(i);
                }
                writer.Serialize();
        ...
        public void ByteArrayReaderWriter_CanReadWritePages()
        {
            ...
            using (var addressStream = new MemoryStream())
            {
                ...
                Assert.IsTrue(BitConverter.ToInt64(reader.Get(0)) == 0);
                Assert.IsTrue(BitConverter.ToInt64(reader.Get(1)) == 1);
                Assert.IsTrue(BitConverter.ToInt64(reader.Get(511)) == 511);
                Assert.IsTrue(reader.Get(512) == ReadOnlySpan<byte>.Empty);

                keyStream.Position = pageSize;
                addressStream.Position = (pageSize / sizeof(long)) * Address.Size;

                reader = new Int64Reader(keyStream, valueStream, addressStream, pageSize: pageSize);
                var v = reader.Get(512);
                Assert.IsTrue(BitConverter.ToInt64(reader.Get(512)) == 512);
        ...
        public void PageWriter_CanWritePages()
        {
            ...
            {
                var testCases = new List<long>();
                using (var writer = new PageWriter<long>(new Int64Writer(keyStream, valueStream, addressStream, pageSize), keyStream, addressStream))
                    for (int i = 0; i < testCount; i++)
                    {
                        writer.TryPut(key: i, value: BitConverter.GetBytes(i));
                        testCases.Add(i);
        ...
        public void PageReader_CanReadPages()
        {
            ...
            using (var addressStream = new MemoryStream())
            {
                ...
                var reader = new PageReader<long>(keyStream, valueStream, addressStream, sizeOfTInBytes: 8, pageSize: pageSize);
        ...
        public void ByteArrayWriter_ReturnsFalseWhenAddingDuplicateKey()
        {
            ...
            using (var addressStream = new MemoryStream())
            {
                var writer = new Int64Writer(keyStream, valueStream, addressStream, 4096);
